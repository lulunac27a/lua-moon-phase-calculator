...
local function getMoonPhase(year, month, day)
    ...
    local phase = ""
    if age < 1.84566                 then phase = "New Moon"
    elseif age < 5.53699             then phase = "Waxing Crescent"
    elseif age < 9.22831             then phase = "First Quarter"
    elseif age < 12.91964            then phase = "Waxing Gibbous"
    elseif age < 16.61096            then phase = "Full Moon"
    elseif age < 20.30229            then phase = "Waning Gibbous"
    elseif age < 23.99361            then phase = "Third Quarter"
    elseif age < 27.68494            then phase = "Waning Crescent"
    else                                  phase = "New Moon"
    end
...
if not year or not month or not day then
    ...
    local max_days = 31
    if month == 4 or month == 6 or month == 9 or month == 11 then max_days = 30
    elseif month == 2 then
        -- Leap year check
